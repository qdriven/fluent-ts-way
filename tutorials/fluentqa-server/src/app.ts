...
router.get('/api', async (ctx) => {
  if (iterID) {
    ...
    ctx.body = {
      ...
      disksIO: {
        rbps: Math.round((fsStats.rx_sec || 0) / 1024),
        wbps: Math.round((fsStats.wx_sec || 0) / 1024),
        tbps: Math.round((fsStats.tx_sec || 0) / 1024)
      },
...
const poll = async () => {
  ...
  try {
    ...
    if (fsStats == null) {
      throw new Error('fsStats cannot be null');
    }
  } catch (e) {
    fsStats = {
      rx: 0,
  ...
  history.push({
    ...
    disksIOrbps: Math.round((fsStats.rx_sec || 0) / 1024),
    disksIOwbps: Math.round((fsStats.wx_sec || 0) / 1024),
    disksIOtbps: Math.round((fsStats.tx_sec || 0) / 1024),
...
(async () => {
  ...
  setInterval(poll, inputArgs.i);

  app.listen(inputArgs.p);

  console.log(`Doppler running on port ${inputArgs.p}`);
})();
